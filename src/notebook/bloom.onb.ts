...
var diagonal = d3.svg
  ...
  .source(function(d) {
    return { y: 100, x: d.from * dy };
  })
  .target(function(d) {
    return { y: (width - bw) / 2, x: (d.to + 0.5) * bw };
  });
...
var vis = svg.append("g").attr("transform", "translate(10,10)");
...
var keyInput = d3.select("#key").on("keyup", function() {
...
function update() {
  ...
  var rect = vis.selectAll("rect").data(buckets(bloom, offLocations));
  rect
    .enter()
    .append("rect")
    .attr("width", bw)
  ...
  rect.attr("class", function(d) {
    return d === 1 ? "on" : d === -1 ? "off" : null;
  });

  var key = vis.selectAll("text.key").data(keys);
  key
    .enter()
    .append("text")
    .attr("class", "key")
  ...
  key.exit().remove();
  key.attr("y", function(d, i) {
    return height / 2 + (i - keys.length / 2) * dy;
  });

  var link = vis.selectAll("path.location").data(links(bloom, keys));
  link
    .enter()
    .append("path")
    .attr("class", "location")
  ...
  link
    ...
    .attr(
      "transform",
      "translate(" + width + "," + height / 2 + ")scale(-1,1)"
    );
  link.exit().remove();
  link.attr("d", diagonal);
  result.text(
    bloom.test(queryText) ? " Probably there." : " Definitely not there."
  );
}
